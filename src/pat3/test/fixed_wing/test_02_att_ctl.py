...
def get_sim_defaults(param_filename, t0=0, tf=5., dt=0.005, trim_args = {'h':0, 'va':12, 'gamma':0.}):
    #trim_args = {'h':0, 'va':15, 'throttle':0., 'phi':np.deg2rad(0.)}
    trim_args = {'h':0, 'va':15, 'gamma':0., 'phi':np.deg2rad(0.)}
    ...
    Xe_ae = dm.state_as_six_dof_euclidian_euler(Xe) # FIXME
...
def run_simulation(ctl_filename, dm, time, Xe, Ue, X0, phi_sp, theta_sp, plot=True):
    ...
    for i in range(1, len(time)):
        ...
        phi_ref[i-1] = ctl.euler_ref.phi_prev
        theta_ref[i-1] = ctl.euler_ref.theta_prev
    ...
    if plot:
        ...
        plt.plot(time, np.rad2deg(theta_ref), label='reference')
        plt.legend()
...
def test_pert_theta(param_filename, ctl_filename, ampl=np.deg2rad(-1.)):
    dm, time, Xe, Ue, phi_sp, theta_sp = get_sim_defaults(param_filename, tf=10.)
    X0 = np.array(Xe); X0[p3_fr.SixDOFEuclidianEuler.sv_theta] += ampl
    return run_simulation(ctl_filename, dm, time, Xe, Ue, X0, phi_sp, theta_sp, plot=True)


def test_step_phi(param_filename, ctl_filename, ampl=np.deg2rad(20.), _dur=10., _p=5.):
    dm, time, Xe, Ue, phi_sp, theta_sp = get_sim_defaults(param_filename, tf=_dur)
    phi_sp = np.array([p3_u.step(_t, a=ampl, p=_p, dt=0) for _t in time])
    return run_simulation(ctl_filename, dm, time, Xe, Ue, Xe, phi_sp, theta_sp, plot=True)

def test_step_theta(param_filename, ctl_filename, ampl=np.deg2rad(1.), _dur=10., _p=10.):
    dm, time, Xe, Ue, phi_sp, theta_sp = get_sim_defaults(param_filename, tf=_dur)
    theta_sp = Xe[p3_fr.SixDOFAeroEuler.sv_theta]+np.array([p3_u.step(_t, a=ampl, p=_p, dt=_p/4.) for _t in time])
    return run_simulation(ctl_filename, dm, time, Xe, Ue, Xe, phi_sp, theta_sp, plot=True)

def test_step_phi_theta(param_filename, ctl_filename, phi_ampl=np.deg2rad(20.), theta_ampl=np.deg2rad(1), _dur=10., _p=5.):
    dm, time, Xe, Ue, phi_sp, theta_sp = get_sim_defaults(param_filename, tf=_dur)
...
def main(param_filename, ctl_filename):
    #test_pert_theta(param_filename, ctl_filename)
    #test_step_phi(param_filename, ctl_filename, ampl=np.deg2rad(45.), _dur=10., _p=10)
    test_step_theta(param_filename, ctl_filename, ampl=np.deg2rad(1.), _dur=30., _p=20. )
    #test_step_theta(param_filename, ctl_filename, ampl=np.deg2rad(5.), _dur=90., _p=70. )
    #test_step_phi_theta(param_filename, ctl_filename, phi_ampl=np.deg2rad(20.), theta_ampl=np.deg2rad(5.), _dur=10., _p=5.)
    plt.show()  
...
if __name__ == "__main__":
    ...
    if 0:
        _param_filename = p3_u.pat_ressource('data/vehicles/cularis.xml')
        _ctl_filename = p3_u.pat_ressource('config/ctl/cularis.yaml')
    if 0:
        _param_filename = p3_u.pat_ressource('data/vehicles/funjet_avl.xml') # not yet :(
    if 1:
        _param_filename = p3_u.pat_ressource('data/vehicles/stormr_avl.xml')
        _ctl_filename = p3_u.pat_ressource('config/ctl/stormr.yaml')
    main(_param_filename, _ctl_filename)
