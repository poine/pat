...
Learning a linear pitch controller
 ...
 originated as a copy of test_02_learn_pil
 adding alpha and/or airspeed (we're about to have this smartprobe, aren't we?)
'''
...
def train_controller(dt, plant_ann, omega_ref=5., xi_ref=0.9, omega_err=10., xi_err=0.7, epochs=20,
    ...
    if force_retrain or not os.path.isfile(full_ann_filename): 
        ...
        eps_k = np.random.uniform(low=-1., high=1., size=(_len,2))             # tracking error at tk
        eps_kp1 = np.array([np.dot(track_err.Ad, _eps_k) for _eps_k in eps_k]) # tracking error at tk+1
...
def test_controller(ref, ann_ctl, plant, Xe, Ue, plant_ann, use_lin_plant=True):
    ''' run the controller on the real world plant and plot results '''
    time =  np.arange(0., 30.01, plant.dt)
    sp, Xr, Xrl = ut.step_vec(time, a0=np.deg2rad(-1), a1=np.deg2rad(1), dt=20., t0=5.), np.zeros((len(time),2)), np.zeros((len(time),2))
    #sp, Xr = np.array([p3_u.step(_t, a=ampl, p=_p, dt=_p/4.) for _t in time]), np.zeros((len(time),2))
    Xl, U, Ul = np.zeros((len(time),2)), np.zeros((len(time),1)), np.zeros((len(time),1))
    ...
    plant.reset(Xp[0])
    if use_lin_plant:
        lin_plant = so_lti.Plant(np.eye(2), np.zeros((2,1)), plant.dt)
        _w = plant_ann.get_layer(name="plant").get_weights()[0]
        lin_plant.Ad, lin_plant.Bd = _w[:2,:].T, _w[2:,:].T
    ki, sum_theta_err = 0.0075, 0
    for k in range(0,len(time)-1):
        if use_lin_plant:
            Xrl[k+1], Ul[k] = controller_compute(ref, ann_ctl, [sp[k]], Xl[k], Xrl[k])
            Xl[k+1] = lin_plant.disc_dyn(Xl[k], Ul[k])
            if k == 2000: Xl[k+1,0] += np.deg2rad(5)
        ...
        if k == 2000:
            Xp[k+1, _s.sv_theta] += np.deg2rad(5); print('pert {}'.format(time[k+1]))
    Up[-1] = Up[-2]
    ...
    plt.plot(time, np.rad2deg(theta_e+Xl[:,0]), label='ided plant')
    plt.legend()
    plt.subplot(5,3,11)
